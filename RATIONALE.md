...
**The free-form scope grammar in CAPABILITY_DECLARATION is a known limitation.** ICS does not define a grammar for directive scopes. Two implementations may express the same constraint using different syntax and both claim compliance. Callers who need to share instructions across systems should define a local scope grammar and document it. This limitation is acknowledged rather than hidden; it will be addressed in a future version.
